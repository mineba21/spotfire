...
테이블: tpm_eqp_loss (설비 정지 이벤트), report_stoploss (집계 리포트)
"""
...
def ask_ai(question: str, page_context: str, selected_bar, sidebar_filters: dict, filter_options=None):
    ...
    context = {
        ...
        # stoploss 전용 스키마 힌트
        "schema_hint": f"""
테이블: {TABLE_EQP_LOSS} (설비 정지 이벤트 로그)
컬럼: yyyymmdd, eqp_id, start_time, end_time, state, param_type, param_name
- state: 정지 원인 코드 (예: MCC_TRIP, PM_SCHEDULED 등)
- param_type / param_name: 인터락/파라미터 정보
- loss_time_min 은 DB 컬럼 없음 — start_time/end_time 차이로 Python 계산되어 응답에 derived 필드로 추가됨.
  ORM 쿼리(filter / values / order_by)에는 절대 사용 불가.

테이블: {TABLE_STOPLOSS_REPORT} (레포트 집계 테이블)
컬럼: yyyy, flag, flagdate, line, sdwt_prod, eqp_id, eqp_model, prc_group, plan_time, stoploss, pm, qual, bm, eng, etc, stepchg, std_time, rd, rank
